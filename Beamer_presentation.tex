...
\begin{frame}
  ...
  \onslide<10-> {With two qubits we can do...}
\end{frame}
...
\begin{frame}
  ...
  \end{block}
  \only<2>{Let us see some examples. (Read the examples in Chapter V section 10 in the notes, page 10 and 11)}
\end{frame}
...
\end{frame}
  ...
  \begin{frame}
    ...
    \centering
\includegraphics[width=5cm]{./img/arkivskab.jpg}
  \end{frame}
  \begin{frame}
...
\item<4-|alert@4> Grover's algorithm needs to be part of a complete algorithm.
  ...
  \begin{frame}
    ...
    Read the example about Grover's algorithm
  \end{frame}
\end{document}